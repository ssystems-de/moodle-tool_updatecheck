...
$string['checkscli'] = 'Moodle updates';
...
$string['setting_checksapiseparator_newline'] = 'Newline';
$string['setting_checksapiseparator_semicolon'] = '; (Semicolon)';
$string['setting_checksapiseparator_slash'] = '/ (Slash)';
$string['setting_checkscliheading'] = 'Checks CLI settings';
$string['setting_checkscliheading_desc'] = 'In addition to the Checks API, this plugin ships its own CLI script admin/tool/updatecheck/cli/checks.php which runs the two checks of this plugin only. It works and looks like the CLI script of Moodle core, but it outputs the details of the checks on all Moodle versions and it is able to output multiple lines of details, for example one line per plugin with an available update. The setting in this section controls how the lines of the details are separated for this CLI script. It does not affect the Checks API, see the previous section.';
$string['setting_checkscliseparator'] = 'Checks CLI separator';
$string['setting_checkscliseparator_desc'] = 'With this setting, you control the separator which is put between the individual pieces of information in the details of the checks when they are run by the CLI script of this plugin. By default, each piece of information is output on its own line. If your monitoring system prefers to get the details in one single line, you can pick one of the other separators, which work in the same way as the Checks API separator.';
$string['setting_checkscommonheading'] = 'Common Checks settings';
$string['setting_checkscommonheading_desc'] = 'The checks of this plugin can be queried in two ways: With the Checks API of Moodle core and with the CLI script of this plugin, see the next two sections. The settings in this section control how the details of the checks are composed and apply to both ways alike.';
$string['setting_checkspluginnameformat'] = 'Plugin name format';
$string['setting_checkspluginnameformat_desc'] = 'With this setting, you control how the plugins are named in the list of available plugin updates which is part of the details of the plugin updates check. This setting does not have any effect on the report page, where the plugin name and the component name are always shown.';
$string['setting_checkssummarylines'] = 'Include summary lines in the details';
$string['setting_checkssummarylines_desc'] = 'With this setting, you control if the details of the checks contain summary lines in addition to the list of available updates: The number of available updates (which repeats the number from the summary, even if it is 0), the number of ignored plugins and of plugins missing from disk with available updates, and the time of the last successful fetch of the update information. These lines allow a monitoring system to get all information from the details alone. If your monitoring system just needs the list of available updates, you can disable the summary lines here. Please note that the CLI script of this plugin does not output the summary lines with the --verbose parameter either if they are disabled. This setting does not affect the details which are shown if the update information is missing or outdated.';
